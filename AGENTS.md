...
- **위험등급 임계값** (`src/data/marineAlertThresholds.ts`에 로직으로 구현되어 있음).
  **2026-09-22 사용자 확정으로 실증사(지오시스템리서치) 발표자료·사업계획서 기준 4단계로 변경** — 이전
  국립수산과학원 5단계(관심 포함)는 더 이상 쓰지 않음:
  - 4단계: 정상(0)·주의(1)·경계(2)·심각(3) — '관심' 없음
  - 사업계획서 [표25] 정의: 정상=31psu 이상·고수온 없음 / 주의=28psu 이하 수괴 서남부 해역 접근 /
    경계=26psu 이하·고수온 연안 10마일 이내 접근 / 심각=대정·한경 양식장 취수구 인근 덮침 + 고수온 동반
  - 관측지점 단일값 판정(근사, 사업계획서도 "단계별 임계치는 수요처 협의 후 확정"):
    염분 정상 >28.0psu · 주의 26.0 초과~28.0 이하 · 경계 24.0~26.0 이하 · **심각 <24.0psu**
  - 수온: 정상 <28.0℃ · 주의(28.0℃ 도달) · 경계(28.0℃↑ 1~2일 지속) · **심각(28.0℃↑ 3일 이상 지속)**
  - 이전 '관심' 5개소는 정상 범위라 영향 양식장 총계가 24 → **19개소**로 바뀜
...
> **실증 3사 착수보고 발표자료 판독 결과 (2026-09-22)**: PDF 슬라이드를 PyMuPDF로 렌더링해 읽고 `/pilot-status`(`src/data/mockPilotStatus.ts`)에 KPI·기능·레거시를 반영함. 아래 4건은 발표자료와 이전 Ground Truth가 달랐던 부분이며, **같은 날 사용자가 "모두 발표자료 기준"으로 확정해 앱 전체와 이 문서 §2에 반영 완료**.
...
### ② 연안 안전관리시스템 (`/coast`)
- 대상지: **1차년도 함덕·삼양 해수욕장 2개소**, 협재 등은 2차년도 추가 실증 후보 (2026-09-22 사용자 확정 — 실증사 올포랜드 발표자료 기준. 이전 "함덕·삼양·협재 3개 관리구역"을 대체. 협재 CCTV는 2차 후보지의 기존 CCTV로 `general` 분류)
...
- 인프라: **효돈천 AIoT 5종 복합 계측망 6개소(스마트폴 포함) — 1차년도 상류 3개소, 2차년도 중·하류 3개소**(2026-09-22 사용자 확정 — 실증사 소다시스템 발표자료 기준. 회의록의 "7개소 + 스마트폴 3개소" 표기를 대체), 기존 서귀포시 자동우량경보시스템 FEP 연계, 레거시 침수정보센서 연계(제주시 66개소·서귀포시 69개소)
- AI 라벨: `Water_Level_High`, `Flood_Imminent`, `Debris_Flow`
- 위험단계: **0단계 안전 · 1단계 경계 · 2단계 대피 · 3단계 중대피** (2026-09-22 사용자 확정 — 기존 효돈천 자동우량경보시스템이 쓰는 4단계, 실증사 사업계획서 <표 11>). 기준값: 제2효례교 수위 0~80·80~150·150~300·300cm 이상, 강우(20분) 0~20·20~30·30~40·40mm 이상. 데이터는 `riverStageCriteria`, 화면 라벨은 `RIVER_LEVEL_LABEL`(`src/data/mockRiver.ts`) — 하천에서는 앱 공통 라벨(주의·경계·심각)을 쓰지 말 것. 색상은 경계=warning, 대피=alert, 중대피=danger
