...
> **2026-09-15 착수보고회 회의록 반영 (사용자 확정)**: ① 저염분수 데이터 소스를 회의 기준(GOCI-II·SMAP 위성, RAMS·NEMO 수치모델, 관측부이)으로 정정하고 저염분수 화면에 섞여 있던 하천·연안 소스(AI CCTV, 수위센서, 강우레이더, 침수 격자, 현장 수동 관측)는 삭제. ③ 하천 인프라·선행시간 표기 수정. 연안 협재는 회의에서 시나리오가 거론되지 않았을 뿐 확정 대상이라 유지.
...
- 인프라: **효돈천 유역 AIoT 계측망 7개소(보전복합) + 스마트폴 3개소**(2026-09-15 착수보고회 회의록 기준 — 기존 "신규 강우레이더 1식 + 신규 수위계" 표기를 대체), 레거시 침수정보센서 연계(제주시 66개소·서귀포시 69개소)
