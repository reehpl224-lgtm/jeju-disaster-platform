...
**폭염 서비스는 이번에 구현하지 않음** — 현업요구사항 문서 4번(열섬지도, 무더위쉼터 안내)은
AGENTS.md §2에서 "이 세 서비스(양식장/연안/하천)가 1차년도 실증 대상의 전부"라고 이미 합의된
범위를 벗어나는 **4번째 서비스 도메인 신설**에 해당합니다. 타입→mock→서브내비→페이지 패턴을 그대로
따라야 하는 큰 작업이라 사용자와 범위 협의 없이 임의로 추가하지 않았습니다. 필요해지면 먼저
상의하세요.
