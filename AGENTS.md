...
- **위험등급 임계값** (`src/data/marineAlertThresholds.ts`에 로직으로 구현되어 있음):
  - 염분: 정상 ≥30.0psu · 관심 28.0~30.0 · 주의 26.0~28.0 · **위험 <26.0psu**
  - 수온: 정상 <25.0℃ · 관심 25.0~28.0℃ · 주의 ≥28.0℃(1~2일 지속) · **위험 ≥28.0℃(3일 이상 지속)**
  - 복합 규칙: **염분 <28.0psu AND 수온 ≥28.0℃ 동시 발생 시 개별 등급과 무관하게 무조건 '위험'로 승격**
    (산소 결핍·삼투압 파괴 가속화 근거)
  - 새 양식장 더미데이터를 추가/수정할 때는 반드시 `classifyMarineRiskLevel()` 함수로 등급을 계산해서
    넣으세요. 손으로 등급을 지어내면 위 임계값과 어긋납니다.
...
- 단계 번호 컨벤션: **1단계=주의, 2단계=경계, 3단계=위험** (아래 위험등급 체계와 동일한 순서)
...
**심각도 순서(낮음→높음): `safe(정상) < caution(관심) < warning(주의) < alert(경계) < danger(위험)`**
...
| `danger` | 위험 | 예전엔 "경보"라는 라벨을 썼으나 지금은 "위험"으로 통일됨 |
...
- **`aquaStages`**(`src/data/mockAqua.ts`)와 `이력·보고서`(`src/data/mockReports.ts`)의
  `incidentRecords`는 `관심/주의/경계/심각/해제`라는 **또 다른 5단계 어휘**를 씁니다. 여기의 "심각"은
  위 3번 표의 "위험"과 다른 용어이고, `정상` 단계도 없습니다. 이건 위험등급이 아니라 **e-SOP 대응
  진행상태**(마지막에 "해제"로 끝남)를 나타내는 별개 개념이라 아직 통일하지 않았습니다. 손대기 전에
  사용자와 먼저 상의하세요.
- MVP 기획 가이드(Manus AI 작성, 2026-09-07) 대조 결과 아직 구현 안 된 항목들 — 급하지 않지만 서비스가
  15개로 늘어나기 전에 검토 예정:
  - 시나리오 선택·재생 UI (지금은 고정 더미데이터만 있음)
  - 화면 상단 `시뮬레이션 데이터` 배지 + 전역 최종갱신시각 표시
