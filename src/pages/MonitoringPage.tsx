  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h1 className="text-xl font-bold text-slate-900">시스템 모니터링</h1>
          <p className="text-xs text-slate-400">마지막 갱신: {monitoringLastSyncedAt}</p>
        </div>
        <button
          type="button"
        ...
        <ul className="flex flex-col divide-y divide-slate-100">
          {incidentLog.map((incident) => (
            <li key={incident.id} className="flex gap-3 py-3">
              ...
              <div>
                <p className="text-sm font-semibold text-slate-700">{incident.title}</p>
                <p className="text-xs text-slate-400">{incident.detail}</p>
                <p className="text-xs text-slate-400">{incident.impact}</p>
              </div>
            </li>
          ))}
        </ul>
      ...
      <Card title="데이터 수집원 및 센서 연결 상태">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {sensorGroups.map((group) => (
            <div key={group.id} className="rounded-lg border border-slate-100 p-3">
              ...
              <p className="mt-2 text-[11px] text-slate-400">마지막 갱신: {group.lastSync}</p>
            </div>
          ))}
        </div>
      </Card>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {serviceHealth.map((service) => (
                <tr key={service.id}>
                  <td className="py-2 font-medium text-slate-700">{service.name}</td>
                  <td className="py-2 text-slate-400">{service.type}</td>
                  <td className="py-2">
                    <RiskBadge level={SERVICE_STATUS_LEVEL[service.status]} />
                  </td>
                  <td className="py-2 text-slate-400">{service.delay}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <Card title="연계 API 운영 현황">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {apiLinks.map((api) => (
                <tr key={api.id}>
                  <td className="py-2 font-medium text-slate-700">{api.name}</td>
                  <td className="py-2 text-slate-400">{api.agency}</td>
                  <td className="py-2">
                    <RiskBadge level={API_STATUS_LEVEL[api.status]} />
                  </td>
                  <td className="py-2 text-slate-400">{api.responseTime}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <ul className="flex flex-col divide-y divide-slate-100">
            {monitoringActionLog.map((log) => (
              <li key={log.id} className="flex gap-3 py-2.5 text-sm">
                <span className="w-12 shrink-0 text-xs text-slate-400">{log.time}</span>
                <p className="text-slate-600">{log.title}</p>
              </li>
            ))}
          </ul>
        </Card>
