  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h1 className="text-xl font-bold text-white">해제 보고</h1>
          <p className="mt-1 text-sm text-white/50">
            {c.caseId} · {c.title}
          </p>
        </div>
        <Link to="/heat" className="inline-flex h-9 items-center rounded-md border border-white/20 px-4 text-xs font-bold text-white hover:bg-white/10">
          폭염 대시보드로 →
        </Link>
      </div>
      ...
      <Card title="사건 상태">
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
          <Field label="유형" value={c.type} />
          <Field label="총 지속 기간" value={`${c.duration} (${c.durationDetail})`} />
          <Field label="운영 현황" value={`${c.agencies} · ${c.agencyDetail}`} />
        </div>
      </Card>
      ...
      <Card title="해제 조건 충족 여부">
        ...
        <p className="mt-3 text-xs font-semibold text-risk-safe">모든 해제 조건 충족 · 최종 승인 완료</p>
      </Card>

      <Card title="최종 해제 보고서">
