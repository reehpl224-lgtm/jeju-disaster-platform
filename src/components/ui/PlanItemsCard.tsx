...
export function PlanItemsCard({
  title,
  subtitle,
  items,
  source = `${MEETING_SOURCE} — 계획·검토 상태이며 실제 측정 결과가 아닙니다.`,
}: {
  title: string
  subtitle?: string
  items: PlanItem[]
  /** 카드 하단 출처 문구 — 기본은 착수보고회 회의록 */
  source?: string
}) {
  return (
    <Card title={title} subtitle={subtitle}>
      ...
      <p className="mt-3 text-[11px] text-white/35">{source}</p>
    </Card>
  )
}
